...
| US               | Compra a CJ desde «Probar una compra»       | ✅ PROBADO 5 sep 2026         | `PRUEBA-20260905184139` ($11,40) pagado del saldo de CJ desde la puerta `/datos/probar-compra` (GitHub): `payBalance {orderId}` → Success, saldo **$150,00 → $138,60**, pedido UNSHIPPED/PENDING con `paymentDate 2026-09-05 20:37:04`. El camino (v1 por orderId; V2 solo de respaldo) está en el módulo y en `pagarConSaldo` de producción. **Falta la compra completa desde la puerta (`comprar`) y una venta real con Stripe que se pague sola** — ver la fila de abajo. |
| US               | Venta con tarjeta → compra a CJ pagada SOLA | ⚠️ SIN PROBAR TRAS EL ARREGLO | El circuito automático (`confirmarYPagarEnCj` → `pagarConSaldo`) lleva el mismo arreglo del 5 sep y el vigilante reintenta las `por_pagar`. Queda probado cuando una venta `MT-…` pagada con tarjeta salga «pagado» en Panel → Pedidos al proveedor sin que nadie pulse nada (la MT-000014 del 5 sep es la primera candidata).                                                                                                                                               |
