...
export default async function PaginaBilletera({
  ...
  const [posicion, movimientos, interno, operador, todos] = await Promise.all([
    obtenerPosicion(comercio),
    listarMovimientosReales(comercio),
    esEquipoInterno(),
    obtenerBilleteraOperador().catch(() => null),
    saldosDeTodosLosComercios().catch(() => []),
  ]);

  /**
   * EL EQUIPO, SIN PEDIR UN COMERCIO, VE A TODOS.
   *
   * Antes veía UNO al azar —el primero que devolviera la base— con su nombre y
   * su saldo, y hasta el botón «Retirar mi dinero», que es del comercio. El
   * dueño lo pilló: la pantalla decía «$0.00 · MEGAYES» mientras el que
   * esperaba su dinero era otro. Una pantalla de dinero que enseña un comercio
   * al azar es peor que no tenerla: se lee «cero» y se cierra tranquilo.
   */
  const vistaDeTodos = interno && !comercio;
  const totalPorPagar = todos.reduce((s, f) => s + f.saldoCentavos, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      {/**
       * EL EQUIPO VE EL TOTAL Y LA LISTA; NO UN COMERCIO AL AZAR.
       *
       * El total sale de sumar las mismas filas que se dibujan debajo, no de
       * una consulta aparte: así el número grande y el detalle no pueden
       * contradecirse, que es la forma más rápida de que nadie se crea
       * ninguno de los dos.
       */}
      {vistaDeTodos ? (
        <section className="space-y-4">
          <article className="rounded-xl border border-slate-200 bg-riel-900 p-6 text-white shadow-sm">
                ...
                {todos.map((f) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      {f.enTramiteCentavos > 0 ? (
                        <p className="text-xs text-tinta-suave">
                          {t("enTramiteFila", {
                            monto: dinero(f.enTramiteCentavos),
                          })}
                        </p>
                      ) : null}
                    </div>
                    ...
                    <Link
                      href={`/panel/billetera?comercio=${f.slug}`}
      ...
      {/* El saldo de UN comercio: el suyo, o el que el equipo pidió ver. */}
      {/* TODO LO DE UN COMERCIO CONCRETO: el suyo, o el que el equipo
          pidió ver con ?comercio=. En la vista de todos no se dibuja —
          si no, debajo del total saldrían los movimientos de uno solo y
          parecerían los de la operación entera. */}
      {vistaDeTodos ? null : (
        <>
              ...
              <dl className="mt-4 grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-lg bg-slate-50 px-4 py-3">
                  ...
                  <dd className="text-xs text-tinta-suave">
                    {t("comision.cuantosRetiros", {
                      n: operador.retiros.length,
                    })}
                  </dd>
                </div>
              </dl>
                ...
                <table className="w-full min-w-[560px] text-sm">
                  <thead>
                    <tr className="text-left text-xs text-tinta-suave">
                      ...
                      <th className="px-4 py-2 text-right font-medium">
                        {t(
                          interno ? "columnas.saldo" : "columnas.saldoComercio",
                        )}
                      </th>
                    </tr>
                  </thead>
