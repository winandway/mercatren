...
CREATE TABLE IF NOT EXISTS `anulaciones_cobro` (
	...
	`motivo` text,
	`origen` text DEFAULT 'socio' NOT NULL,
	`anulado_en` integer NOT NULL,
...
CREATE INDEX IF NOT EXISTS `idx_banners_activo_mercado` ON `banners` (`activo`,`mercado`);
CREATE INDEX IF NOT EXISTS `idx_banners_tienda` ON `banners` (`tienda_id`);
...
CREATE TABLE IF NOT EXISTS `cobros_cadena` (
	`cobro_id` text PRIMARY KEY NOT NULL,
...
CREATE INDEX IF NOT EXISTS `idx_comentarios_video` ON `comentarios_video` (`video_id`,`estado`);
...
CREATE TABLE IF NOT EXISTS `contactos_busqueda` (
...
CREATE INDEX IF NOT EXISTS `idx_correcciones_pago` ON `correcciones_pago` (`pago_zelle_id`);
...
CREATE TABLE IF NOT EXISTS `embeddings_producto` (
...
CREATE INDEX IF NOT EXISTS `idx_embeddings_producto_mercado` ON `embeddings_producto` (`mercado`);
...
CREATE TABLE IF NOT EXISTS `me_gusta_video` (
	`video_id` text NOT NULL,
	`usuario_id` text NOT NULL,
	`creado_en` integer DEFAULT (unixepoch()) NOT NULL,
...
CREATE INDEX IF NOT EXISTS `idx_me_gusta_video` ON `me_gusta_video` (`video_id`);
...
CREATE INDEX IF NOT EXISTS `idx_partes_del_cobro_grupo` ON `partes_del_cobro` (`grupo`);
...
CREATE TABLE IF NOT EXISTS `renglones_proveedor` (
	`id` text PRIMARY KEY NOT NULL,
...
CREATE INDEX IF NOT EXISTS `idx_renglones_proveedor` ON `renglones_proveedor` (`pedido_proveedor_id`);
...
CREATE TABLE IF NOT EXISTS `tarifas_casillero` (
...
CREATE TABLE IF NOT EXISTS `tarifas_del_cobro` (
	`cobro_id` text PRIMARY KEY NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS `videos_tienda_slug_unique` ON `videos_tienda` (`slug`);
...
CREATE INDEX IF NOT EXISTS `idx_visitas_mercado` ON `visitas` (`mercado`);
CREATE INDEX IF NOT EXISTS `idx_visitas_visitante` ON `visitas` (`visitante`);
...
VALUES ('tienda-bley-ferreteria', 'bley-ferreteria', 'Ferremateriales Bley C.A', 'activa', 300, 'VE', NULL, NULL, 1789742860, 1789742860)
ON CONFLICT(id) DO NOTHING;

INSERT INTO billeteras (id, tienda_id, saldo_centavos, moneda, proveedor, estado, creado_en)
VALUES ('billetera-bley-ferreteria', 'tienda-bley-ferreteria', 0, 'USD', 'tokiia', 'activa', 1789742860)
